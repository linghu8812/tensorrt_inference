...
MiniFASNet::MiniFASNet(const std::string &config_file) {
    YAML::Node root = YAML::LoadFile(config_file);
    YAML::Node config = root["MiniFASNet"];
...
MiniFASNet::~MiniFASNet() = default;

void MiniFASNet::LoadEngine() {
...
bool MiniFASNet::InferenceFolder(const std::string &folder_name) {
...
void MiniFASNet::EngineInference(const std::vector<std::string> &image_list, const int &outSize, void **buffers,
...
//            cv::cvtColor(src_img, src_img, cv::COLOR_BGR2RGB);
        ...
        if (batch_id == BATCH_SIZE or index == image_list.size())
        {
            ...
            {
                auto position = std::max_element(out + i * outSize, out + (i + 1) * outSize);
                std::string result = position - (out + i * outSize) == 1 ? "real" : "fake";
...
std::vector<float> MiniFASNet::prepareImage(std::vector<cv::Mat> &vec_img) {
