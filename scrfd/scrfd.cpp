...
scrfd::scrfd(const std::string &config_file) {
    ...
    mean = config["mean"].as<float>();
    std= config["std"].as<float>();
...
bool scrfd::InferenceFolder(const std::string &folder_name) {
...
void scrfd::EngineInference(const std::vector<std::string> &image_list, const int &outSize, void **buffers,
...
//                cv::cvtColor(org_img, org_img, cv::COLOR_BGR2RGB);
                for(const auto &rect : rects)
                {
                    ...
                    if (rect.has_mask) {
                        ...
                        cv::putText(org_img, "mask", cv::Point(rect.face_box.x,rect.face_box.y + 15),
                                    cv::FONT_HERSHEY_COMPLEX, 0.7, color, 2);
                    } else {
                        color = cv::Scalar(255, 0, 0);
                    }
                    cv::putText(org_img, name, cv::Point(rect.face_box.x,rect.face_box.y - 5),
                            cv::FONT_HERSHEY_COMPLEX, 0.7, color, 2);
                    cv::Rect box(rect.face_box.x, rect.face_box.y, rect.face_box.w, rect.face_box.h);
...
std::vector<float> scrfd::prepareImage(std::vector<cv::Mat> &vec_img) {
...
std::vector<std::vector<scrfd::FaceRes>> scrfd::postProcess(const std::vector<cv::Mat> &vec_Mat,
    ...
    for (const cv::Mat &src_img : vec_Mat)
    {
        ...
        for (int item = 0; item < result_matrix.rows; ++item) {
            ...
            if(current_row[0] > obj_threshold){
                ...
                headbox.face_box.w = ((bbox[0] + bbox[2]) * anchor[2]) * ratio;
                headbox.face_box.h = ((bbox[1] + bbox[3]) * anchor[2]) * ratio;
...
void scrfd::NmsDetect(std::vector<FaceRes> &detections) {
...
float scrfd::IOUCalculate(const scrfd::FaceBox &det_a, const scrfd::FaceBox &det_b) {
